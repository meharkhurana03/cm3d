...
class Calibration(object):
    ...
    def __init__(self, calib_filepath, device='cpu', from_video=False):
        self.device = device
        if from_video:
        ...
        self.P = calibs["P2"]
        self.P = self.P.view([3, 4])
        # Rigid transform from Velodyne coord to reference camera coord
        self.V2C = calibs["Tr_velo_to_cam"]
        self.V2C = self.V2C.view([3, 4])
        self.C2V = inverse_rigid_trans(self.V2C)
        # Rotation from reference camera coord to rect camera coord
        self.R0 = calibs["R0_rect"]
        self.R0 = self.R0.view([3, 3])
        ...
        with open(filepath, "r") as f:
            for line in f.readlines():
                ...
                try:
                    ...
                    data[key] = torch.from_numpy(data[key]).to(self.device, dtype=torch.float32)
                except ValueError:
                    pass
    ...
    def project_velo_to_ref(self, pts_3d_velo):
        ...
        return torch.matmul(pts_3d_velo, self.V2C.transpose(0, 1))

    def project_ref_to_velo(self, pts_3d_ref):
        pts_3d_ref = self.cart2hom(pts_3d_ref)  # nx4
        return torch.matmul(pts_3d_ref, self.C2V.transpose(0, 1))

    def project_rect_to_ref(self, pts_3d_rect):
        """ Input and Output are nx3 points """
        return torch.matmul(torch.linalg.inv(self.R0), pts_3d_rect.T).T

    def project_ref_to_rect(self, pts_3d_ref):
        """ Input and Output are nx3 points """
        return torch.matmul(self.R0, pts_3d_ref.T).T
...
def inverse_rigid_trans(Tr):
    """ Inverse a rigid body transform matrix (3x4 as [R|t])
        [R'|-R't; 0|1]
    """
    inv_Tr = torch.zeros_like(Tr).to(Tr.device, dtype=Tr.dtype) # 3x4
    inv_Tr[0:3, 0:3] = Tr[0:3, 0:3].transpose(0, 1)
    inv_Tr[0:3, 3] = torch.matmul(-Tr[0:3, 0:3].transpose(0, 1), Tr[0:3, 3])
    return inv_Tr
